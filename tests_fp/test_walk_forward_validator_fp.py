...
from walk_forward_validator_fp import StatResult
...
def test_apply_purge_embargo_embargoes_test_rows_near_boundary():
    ...
    out_train, out_test, n_purged, n_embargoed = wfv.apply_purge_embargo(train, test, "date", embargo_days=3)
...
def test_walk_forward_validate_purge_days_reduces_train_and_reports_count():
    n = 100
    df = pd.DataFrame({"date": pd.date_range("2020-01-01", periods=n, freq="D"), "excess_pct": [2.0] * n})
    baseline = wfv.walk_forward_validate(df, "date", lambda d: wfv.stat_vs_zero(d["excess_pct"]))
    purged = wfv.walk_forward_validate(df, "date", lambda d: wfv.stat_vs_zero(d["excess_pct"]), purge_days=10)
...
def test_walk_forward_validate_embargo_days_reduces_test_and_reports_count():
    ...
    df = pd.DataFrame({"date": pd.date_range("2020-01-01", periods=n, freq="D"), "excess_pct": [2.0] * n})
    baseline = wfv.walk_forward_validate(df, "date", lambda d: wfv.stat_vs_zero(d["excess_pct"]))
    embargoed = wfv.walk_forward_validate(df, "date", lambda d: wfv.stat_vs_zero(d["excess_pct"]), embargo_days=10)
